"""
compare_safety_sarsa.py
-----------------------
Aggregates SARSA safety margins across scan depths and saves a
...
def extract_safety(scan, data_dir, theta=60):
    """Read Avg_Safety_Margin row from SARSA History CSV for *scan* depth."""
    path = os.path.join(data_dir, "SARSA", f"Scan_{scan}",
                        f"SARSA_Th{theta}_Scan{scan}_History.csv")
...
def generate_comparison(data_dir=None, output_path=None, thresholds=None, theta=60):
    """Build and save a safety-margin comparison CSV for SARSA."""
    if data_dir is None:
    ...
    print(f"[DONE] SARSA safety comparison -> {output_path}")
